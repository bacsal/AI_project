...
def ML():
    # 사용자 입력 받기
    my_champion = entry1.get()
    enemy_champion = entry2.get()
    ...
    if matching_rows.empty:
        ...
        tkinter.messagebox.showinfo("결과", f"{my_champion}이(가) {enemy_champion}과(와) 겨룰 때 적합한 템트리는 {result_item}이고, 이로 인한 승률은 [ {result_win_rate * 100:.2f}% ] 로 예측됩니다.")

        entry1.delete(0, "end")
        entry2.delete(0, "end")

label1 = Label(tk,text='내 챔피언').grid(row=0, column=0)
label2 = Label(tk,text='상대 챔피언').grid(row=1,column=0)
...
btn1 = Button(tk,text='확인!',bg='black',fg='white',command=ML).grid(row=2,column=2)
